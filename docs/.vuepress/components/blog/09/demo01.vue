<template>
  <div>
    ...
    <ul>
      <li v-for="drink in filteredArrayDrinks">
        {{drink}}
      </li>
    </ul>
    <ul>
      <li v-for="drink in filteredMapDrinks">
        {{drink}}
      </li>
    </ul>
    <p>{{url}}</p>
    ...
    <button @click="includes()">includes()</button>
    <button @click="toArray()">toArray()</button>
    <button @click="assign()">アルコール飲み放題</button>
    <button @click="toMap()">toMap()</button>
...
export default {
  ...
  methods: {
    push: function() {
      ...
      this.filteredDrinks = this.drinks.push({id:id, name:"コーラ", price: 120})
    },
    sort: function() {
